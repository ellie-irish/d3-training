...
// The data circles appearing and disappearing transition
        ...
        function updateData() {
            ...
            if (this.checked) {
                ...
                var number = +inputValue.substr(-1);

                   var maxElevation = d3.select('#maxElevation').node().value

                   if (maxElevation === '') {
                    maxElevation = elevations.domain()[1]
                   }

                   else if (isNaN(maxElevation)) {
                    alert('this is not a number')
                    return;
                   }

                   var minElevation = d3.select('#minElevation').node().value

                   if (minElevation === '') {
                    minElevation = elevations.domain()[0]
                   }
                   else if (isNaN(minElevation)) {
                    alert('this is not a number')
                    return;
                   }
                    ...
                    g
                        .selectAll('circle')
                        .filter(function(d) {
                            return d.stationClass === number && d.elevation <= maxElevation && d.elevation >= minElevation; })
                        .data(location.filter(function(d) {
                            return d.stationClass === number && d.elevation < maxElevation && d.elevation > minElevation; }))
                        .enter()
                        .append('circle')
                        .attr('cx', function(d) {
